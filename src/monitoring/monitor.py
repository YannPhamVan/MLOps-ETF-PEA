...
def generate_monitoring_reports(reference_path, current_path, output_dir):
    ...
    try:
        ...
        print(
            f"[INFO] Loaded model version {latest_versions[0].version} for monitoring."
        )
    ...
    features = [
        ...
        if col not in ["target", "prediction", "Date", "ticker"]
    ]

    column_mapping = ColumnMapping(
        target="target", prediction="prediction", numerical_features=features
    )
